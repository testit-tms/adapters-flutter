...
1. Find the sync-storage version for your adapter release: check out the release tag and look for the `_version` value in `sync_storage_runner.dart`
...
4. Run your tests as usual; the adapter will pick up and run the version specified in `_version`.
...
| Mode of import type selection when launching autotests (**It's optional**). Default value - true. true - create/update each autotest in real time; false - buffer results and flush in batch at tearDownAll (call `tmsFlushPendingResultsAsync()` explicitly in CI if needed)                                                         | importRealtime                    | TMS_IMPORT_REALTIME                        | tmsImportRealtime                    |
